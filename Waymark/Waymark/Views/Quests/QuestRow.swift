...
struct QuestRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 10) {
                Text(quest.name).font(Theme.bodyMed)
        ...
        .padding(16).glass(strong: hovering)
    ...
    @ViewBuilder private var quickActions: some View {
        HStack(spacing: 12) {
            if quest.hasNextStep {
                Button { withAnimation(.spring(response: 0.5, dampingFraction: 0.85)) { store.completeNextStep(quest) } }
                    label: { Image(systemName: "checkmark").font(.system(size: 11, weight: .semibold)) }
                    .buttonStyle(.plain).foregroundStyle(Theme.accent).help("Mark this step done")
            }
            if let next = quest.stage.next {
                Button { withAnimation(.spring(response: 0.5, dampingFraction: 0.85)) { store.advanceStage(quest) } }
