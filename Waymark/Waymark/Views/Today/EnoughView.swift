...
/// No next twelve tasks. A recap of what mattered, and permission to go live.
/// "More" exists but never dangles — it takes a deliberate tap to reveal.
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    VStack(spacing: 8) {
                        ForEach(store.todaysProgress) { ev in
                            HStack(spacing: 10) {
                                Image(systemName: ev.kind == .advancedStage ? "arrow.up.right.circle.fill" : "checkmark.circle.fill")
                                    .foregroundStyle(ev.axis.color)
            ...
            VStack(alignment: .leading, spacing: 12) {
                if revealMore {
                    if store.hasActionableQuests {
                        Text("If you truly want to keep going:")
                            .font(Theme.caption).foregroundStyle(Theme.inkFaint)
                        Button {
                            withAnimation { store.reopenToday() }
                        } label: {
